...
class MainScene extends Scene<'main', RockClimber> {
    ...
    async preload() {
        await PIXI.Assets.load([
            ...
            {alias: 'logo', src: 'assets/main/logo/logo_104.png'},
    ...
    async create(): Promise<void> {
        const slot = new Slot(3, 5, {size: {width: 144, height: 144}, gap: {x: 17, y: 17}});
        onResize(({game}) => {
            slot.x = game.width / 2;
            slot.y = 95;
        ...
        const gui = new Gui();
        this.game.stage.addChild(gui);

        gui.startClicked.add(async () => {
...
class BonusScene extends Scene<'bonus', RockClimber> {
    constructor() {
        super('bonus');
    }
}
...
export class RockClimber extends Game<[MainScene, BonusScene]> {
    constructor() {
        super([new MainScene(), new BonusScene()]);


        // const a = this.sceneManager.getScene("main").
